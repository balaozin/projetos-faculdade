...
show databases;
-- Criação das tabelas do Banco de Dados LocadoraVeiculos -- 
...
insert into Pagamento (idPagamento, forma, dataPagamento, valorTotal, estado) values
     ...
     INSERT INTO Pagamento (idPagamento, forma, dataPagamento, valorTotal, estado) VALUES
(16, 'Pix', '2025-01-13', 980.00, 'Pago'),
(17, 'Cartão', '2025-01-16', 1220.00, 'Pago'),
(18, 'Pix', '2025-01-18', 780.00, 'Pago'),
(19, 'Dinheiro', '2025-01-19', 640.00, 'Pago'),
(20, 'Cartão', '2025-01-21', 1120.00, 'Pago');
...
select modelo, marca, count(*) as quantidade_de_locacoes
from Veiculo v
...
select nome, sum(valorTotal) as valor_total_devido
from Cliente, Locacao, Pagamento
where Cliente.idCliente = Locacao.idCliente
and Locacao.idPagamento = Pagamento.idPagamento
and Pagamento.estado = 'Pendente'
group by nome
order by nome;
...
UPDATE Pagamento
SET estado = 'Pendente'
WHERE idPagamento IN (16, 18, 19, 20);